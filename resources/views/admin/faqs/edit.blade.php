...
@section('page-title', 'SSS Düzenle')

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">SSS Düzenle</h5>
    </div>
    <div class="card-body">
        @if($errors->any())
        ...
        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST">
            ...
            @method('PUT')
            @php
                $questionTranslations = $faq->getQuestionTranslations();
                $answerTranslations = $faq->getAnswerTranslations();
            @endphp
            ...
            <div class="mb-3">
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="question_tr" role="tabpanel">
                        <textarea class="form-control" name="question_tr" rows="2" required>{{ old('question_tr', $questionTranslations['tr'] ?? '') }}</textarea>
                    </div>
                    <div class="tab-pane fade" id="question_en" role="tabpanel">
                        <textarea class="form-control" name="question_en" rows="2" required>{{ old('question_en', $questionTranslations['en'] ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="answer_tr" role="tabpanel">
                        <textarea class="form-control" name="answer_tr" rows="5" required>{{ old('answer_tr', $answerTranslations['tr'] ?? '') }}</textarea>
                    </div>
                    <div class="tab-pane fade" id="answer_en" role="tabpanel">
                        <textarea class="form-control" name="answer_en" rows="5" required>{{ old('answer_en', $answerTranslations['en'] ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control" name="sort_order" value="{{ old('sort_order', $faq->sort_order) }}">
                </div>
                <div class="col-md-6 mb-3">
                    <div class="form-check mt-4">
                        <input type="checkbox" class="form-check-input" name="is_active" id="is_active" {{ old('is_active', $faq->is_active) ? 'checked' : '' }}>
